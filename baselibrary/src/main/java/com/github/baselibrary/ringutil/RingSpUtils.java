...
public class RingSpUtils {
    ...
    public RingSpUtils(Context ctx) {
        this(ctx, NATION);
    }

    public RingSpUtils(Context ctx, String file) {
        mSp = ctx.getSharedPreferences(file, Context.MODE_PRIVATE);
    }

    public static RingSpUtils getInstance(Context ctx) {
        if (instance == null) {
            instance = new RingSpUtils(ctx);
        }
        return instance;
    }
...
//    }
    ...
    public void saveCollectionAlbums(String str) {
        put("CollectionAlbums", TextUtils.isEmpty(str) ? "[]" : str);
    }
